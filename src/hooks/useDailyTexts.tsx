    ...
    const fetchTexts = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from('daily_texts')
          .select('text_type, text_content, duration_minutes')
          .eq('day_number', dayNumber)
          .in('text_type', ['fase_descricao', 'dia_descricao', 'video_duracao', 'hipnose_duracao']);
        ...
        const videoDuracao = data?.find(item => item.text_type === 'video_duracao')?.duration_minutes || null;
        const hipnoseDuracao = data?.find(item => item.text_type === 'hipnose_duracao')?.duration_minutes || null;

        setTexts({
          fase_descricao: faseDescricao,
          dia_descricao: diaDescricao,
          video_duracao: videoDuracao,
          hipnose_duracao: hipnoseDuracao
        });
      } catch (error) {
